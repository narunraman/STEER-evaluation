...
# Local packages
from utils.response_utils import HF_RESPONSE, get_explanation_probs, parse_response, compute_ece, get_correct, get_true_labels
from utils.question_utils import reconstruct_context, build_prefix, get_test_questions, permute_answer, convert_probabilities
from utils.parsing_utils import find_answer_letter
from utils.utils import get_option_letters, print_chat, read_as_defaultdict, load_dfs, get_chat_type
from utils.model_utils import load_model_tokenizer, GPTClient
...
def get_response_hf(model, tokenizer, device, prefix, questions, question_type, options_lst, chat_type):
    ...
    for i, question in enumerate(questions):
        ...
        if question_type == 'mc' or question_type == 'mc-separate':
            answer, probs = HF_RESPONSE[question_type](model, tokenizer, context + '\n' + question, options_lst[i], device)
            outputs.append(answer)
            parsed_results.append(['', answer, probs])

        elif question_type == 'explanation':
            output = HF_RESPONSE[question_type](model, tokenizer, context + '\n' + question, device)
            outputs.append(output)
            ...
            output = HF_RESPONSE['explanation'](model, tokenizer, context + '\n' + question, device)
            outputs.append(output)

        elif i % 2 == 1 and (question_type == 'sequential-hidden' or question_type == 'sequential-shown'):
            answer, probs = HF_RESPONSE['mc'](model, tokenizer, context + '\n' + question, options_lst[i], device)
            outputs.append(answer)
            parsed_results.append([outputs[i-1], answer, probs])
...
def create_results_dict(params, task_name, model_name, base_id, sub_id, permuted_answer, model_answer, model_explanation, probabilities, permutations):
    ...
    # results['normalized_accuracy'] = results['accuracy'] - get_random_acc(task_name)
    return results
...
def eval_models(args, api, device=None):
    ...
    for model_name in model_names:
        ...
        if device:
            model, tokenizer = load_model(device, args, model_name)
            if not model:
                continue
            else:
